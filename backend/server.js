...
const io = new Server(httpServer, {
  cors: {
    origin: [
      ...
      // Add your cPanel domain here, e.g.:
...
app.use(cors({
  origin: function(origin, callback) {
    const allowedOrigins = [
      ...
      'http://localhost:5173',
      'https://taskflow-nine-phi.vercel.app',
      process.env.CLIENT_URL // Add production domain from env
    ].filter(Boolean); // Remove undefined/null values
...
// Serve static files from frontend build (for production/cPanel deployment)
// The frontend dist files should be placed in backend/public folder
