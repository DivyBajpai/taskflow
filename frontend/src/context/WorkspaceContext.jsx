...
export const WorkspaceProvider = ({ children }) => {
  ...
  const switchWorkspace = async (workspaceId) => {
    try {
      ...
      if (response.data?.success) {
        console.log('Workspace switched successfully:', response.data);
        updateWorkspace(response.data.workspace);
        ...
        if (storedUser) {
          try {
            ...
            user.workspace = response.data.workspace;
        ...
        console.error('Workspace switch failed:', response.data);
  ...
  const fetchAllWorkspaces = async () => {
    try {
      ...
      const response = await api.get('/auth/my-workspaces');
      
      if (response.data?.success) {
        console.log('Workspaces fetched:', response.data.workspaces);
        setAllWorkspaces(response.data.workspaces);
        localStorage.setItem('allWorkspaces', JSON.stringify(response.data.workspaces));
